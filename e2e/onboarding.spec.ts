...
/**
 * De kluspool toont een onboarding-blok dat naar de handleiding leidt (welkom-variant bij een lege
 * kluspool, compacte tip als er klussen zijn). "Niet meer tonen" verbergt het en onthoudt dat per
 * toestel (localStorage), dus ook na herladen. Draait onder de monteur-sessie.
 */
test.use({ storageState: "e2e/.auth/monteur.json" });

test("kluspool-onboarding leidt naar de handleiding en onthoudt 'niet meer tonen'", async ({ page }) => {
  await page.goto("/");
