...
| Rapport-herindeling: meldingen bovenaan (sectie 1) + gekleurde meldingen-balk (rood spoed / oranje gewoon / groen geen), overzicht in sectie-volgorde, foto-nummering meldingen-eerst; PDF + in-app weergave gelijk | U | rapport-indeling.test (meldingenBalk status/tekst, fotoGroepen/platteFotoLijst nummering), rapport.test (PDF rendert geldig) | U groen; visueel door Rein |
| Foto's downloaden: knop in de PDF alleen in de ZAAK-versie (niet klant), absolute link via APP_URL, weg zonder foto's/APP_URL | U | rapport.test (fotoDownloadLink + zaak-wel/klant-niet-annotatie), rapport-indeling.test (fotoDownloadEntries namen/index) | groen |
| Foto-downloadpagina (publiek, opdracht-id) + zip-route: selectie/alles, 404 onbekend/geen foto's, 400 lege selectie, 502 alle fetches mislukt | U | api/klus/[id]/fotos/zip/route.test (statuscodes + zip-magic + selectie) | U groen; E door Rein |
